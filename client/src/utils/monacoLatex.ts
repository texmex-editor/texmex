...
export function ensureLatexLanguageRegistered(): void {
  if (isLatexLanguageRegistered) {
    return;
  }

  if (!monaco.languages.getLanguages().some((entry) => entry.id === LATEX_LANGUAGE_ID)) {
  ...
  monaco.languages.setLanguageConfiguration(LATEX_LANGUAGE_ID, conf);
  monaco.languages.setMonarchTokensProvider(LATEX_LANGUAGE_ID, language);
